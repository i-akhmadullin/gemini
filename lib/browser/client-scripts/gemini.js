(function(window) {
    ...
    function prepareScreenshotUnsafe(selectors, opts) {
        var result = prepareBodyScreenshot(selectors, opts);
        if (result.error) {
            return result;
        }

        function adjustScrolling(rect) {
        ...
        if (opts.needsScrolling) {
            window.scrollTo(result.captureArea.left, result.captureArea.top);
            result.captureAre = adjustScrolling(result.captureArea);
            result.ignoreAreas = result.ignoreAreas.map(adjustScrolling);
        }

        result.captureArea = result.captureArea.serialize();
        result.ignoreAreas = result.ignoreAreas.map(function(area) {
    ...
    function prepareBodyScreenshot(selectors, opts) {
        ...
        return {
            ...
            ignoreAreas: opts.ignoreSelectors.map(function(selector) {
                return getElementCaptureRect(selector).round();
            }),
            coverage: coverage,
    ...
    function getElementCaptureRect(element) {
        ...
        for (var i = 0; i < pseudo.length; i++) {
            css = window.getComputedStyle(element, pseudo[i]);
